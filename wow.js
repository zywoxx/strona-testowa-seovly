...
;(function(){
  ...
  ss.textContent=
    '.svc-card,.why-card,.dk-card,.bento-item{position:relative}'+
    '.sp-glow{position:absolute;inset:0;pointer-events:none;opacity:0;transition:opacity .4s;'+
    'background:radial-gradient(350px circle at var(--sx,50%) var(--sy,50%),rgba(255,255,255,.10),transparent 65%);'+
    'z-index:0;border-radius:inherit}';
  document.head.appendChild(ss);
  function run(){
    document.querySelectorAll('.svc-card,.why-card,.dk-card,.bento-item').forEach(function(el){
      if(el.querySelector('.sp-glow'))return;
      ...
      el.addEventListener('mouseleave',function(){g.style.opacity='0';});
    });
  }
